...
def classify_one_multi_crop(sess, label_list, softmax_output, coder, images, image_file, writer):
    try:
        ...
        print('Guess @ 1 %s, prob = %.2f' % best_choice)
        gender = best_choice[0]
        print(gender)
        if (gender == 'M'):
        #if any("M" in gender for gender in best_choice):
            print('Male detected')
            os.system("aplay --device=hw:1,0 who-let-the-dogs-out-ringtone.wav")
        else:
            print('Female detected')
            os.system("aplay --device=hw:1,0 beautiful.wav")    
